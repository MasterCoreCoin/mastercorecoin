...
MasterCoreCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<mastercorecoin_address\> \<monthly_payment_mastercorecoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _5000_ MAC, _12_ times over the course of a year totaling _60000_ MAC.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<mastercorecoin_address\> \<monthly_payment_mastercorecoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `5000` MAC.
 ...
 * vote-many          - Vote on a MasterCoreCoin initiative
 * vote-alias         - Vote on a MasterCoreCoin initiative
 * vote               - Vote on a MasterCoreCoin initiative/budget
