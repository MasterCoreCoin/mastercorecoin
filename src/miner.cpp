...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("MasterCoreCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("MasterCoreCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("MasterCoreCoinMiner started (POS=%s)\n", (fProofOfStake ? "true" : "false") );
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mastercorecoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running MasterCoreCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
